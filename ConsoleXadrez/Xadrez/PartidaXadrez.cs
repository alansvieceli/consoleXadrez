...
namespace Xadrez {
    class PartidaXadrez {
        ...
        private void colocarPecas() {
            ...
            colorNovaPeca(new Rei(this.tabuleiro, Cor.Verde, this), 'e', 8);
            ...
            colorNovaPeca(new Rei(this.tabuleiro, Cor.Vermelha, this), 'e', 1);
        ...
        private Peca executarMovimento(Posicao posOrigem, Posicao posDestino) {
            ...
            if (p is Rei && posDestino.coluna == posOrigem.coluna + 2) {
                Posicao origemT = new Posicao(posOrigem.linha, posOrigem.coluna + 3);
                Posicao destinoT = new Posicao(posOrigem.linha, posOrigem.coluna + 1);
            ...
            if (p is Rei && posDestino.coluna == posOrigem.coluna - 2) {
                Posicao origemT = new Posicao(posOrigem.linha, posOrigem.coluna - 4);
                Posicao destinoT = new Posicao(posOrigem.linha, posOrigem.coluna - 1);
                Peca T = tabuleiro.removePeca(origemT);
                T.incrementarQtdeMovimentos();
                tabuleiro.addPeca(T, destinoT);
            ...
            if (p is Peao) {
                if (posOrigem.coluna != posDestino.coluna && capturada == null) {
                    Posicao posP;
                    ...
                    capturada = tabuleiro.removePeca(posP);
                    capturadas.Add(capturada);
                }
            }
        ...
        private void desfazMovimento(Posicao posOrigem, Posicao posDestino, Peca capturada) {
            ...
            // #jogadaespecial en passant
            if (p is Peao) {
                if (posOrigem.coluna != posDestino.coluna && capturada == vulneravelEnPassant) {
                    Peca peao = tabuleiro.removePeca(posDestino);
                    Posicao posP;
                    if (p.cor == Cor.Vermelha) {
                        posP = new Posicao(3, posDestino.coluna);
                    } else {
                        posP = new Posicao(4, posDestino.coluna);
                    }
